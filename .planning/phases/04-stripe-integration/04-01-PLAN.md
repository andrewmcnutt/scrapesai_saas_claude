...
must_haves:
  truths:
    - "Stripe payment processing is configured and ready for webhook and checkout integration"
    - "Subscription data persists with per-user isolation via RLS"
    - "Webhook event deduplication prevents double-processing of Stripe events"
    - "New users automatically receive 3 free credits on signup (CRED-01)"
...
<task type="auto">
  ...
  <action>
    ...
    4. **Free tier credit seeding trigger (CRED-01):**
       New users must start with 3 free credits. Create a trigger on `auth.users` that inserts a credit_transaction on user signup:
       ```sql
       -- Seed 3 free credits for new users (CRED-01)
       ...
       ```
       - Uses the existing `signup_bonus` type already in the credit_transactions CHECK constraint
       - `SECURITY DEFINER` is required because the trigger fires on `auth.users` but inserts into `credit_transactions` which has RLS enabled — the trigger function must run with the definer's privileges (superuser/postgres) to bypass RLS
       - Metadata includes `reason: initial_free_credits` for audit clarity
       - This ensures every new user starts with balance=3, so the free tier "3 lifetime generations" promise (CRED-01) works from day one

    This migration must be applied manually via Supabase SQL Editor (consistent with Phase 1 pattern — webhook not triggered).

    **NOTE for existing users:** If there are existing users in auth.users who never received signup credits, run a one-time backfill after applying this migration:
    ```sql
    INSERT INTO credit_transactions (user_id, amount, type, metadata)
  ...
  <verify>
    ...
    - Contains `seed_free_credits` function and `on_auth_user_created` trigger
    - Contains backfill query for existing users (as a comment or separate block)
  </verify>
  <done>
    Migration file creates subscriptions table with RLS, stripe_processed_events table, indexes, updated_at trigger, and free credit seeding trigger (CRED-01 — 3 credits on signup). Ready for manual application via Supabase SQL Editor.
  </done>
</task>
...
- SQL migration file ready for manual application (includes free credit seeding trigger for CRED-01)
- .env.example documents all required Stripe environment variables
- New users will receive 3 free credits on signup via database trigger
</success_criteria>
